...
function SoundEngine:playSound(SoundPath)
		...
		local Channels = audio.play(SoundInstance.Instance,{channel=availableChannel, loops=SoundInstance.Loops})
...
function SoundEngine:playStream(StreamPath)
	    ...
	    local Channels = audio.play(StreamInstance.Instance,{channel=availableChannel, loops=StreamInstance.Loops})
		self.isCurrentPlayingSound=true
		availableChannel = nil
		return Channels
end
