...
function Object.create(params)
	...
	elseif(params.type == "TEXT") then
		...
		object.object = display.newText(finalText,params.x,params.y,native.systemFont,params.textSize *1.4)
...
function Object:startInteractions()
	...
	if(self.isLocked == false) then 
		...
		if(self.isStarted == false) then 
			...
			if(self.type == "SPRITE") then 
				if(self.currentSequence) then 
					self.object:prepare(self.currentSequence)
					self.object:play()
				end
			end
		...
		if(self.type == "TEXT") then
			local finalText = rosetta:getString(self.text)
			if(finalText == nil or finalText =="NO STRING") then finalText = self.text end
