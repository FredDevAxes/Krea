...
function TilesMap:tileVisibleTest(t,x,y)
	...
	--Save current Sequence and frame
	if(t.texture and t.currentIndexInTabs) then 
		if(t.texture.sequence) then 
			self.tabCurrentTextureFrameForTile[t.currentIndexInTabs][t.texture.sequence] = t.texture.currentFrame
		end
	end
	...
	if(x >= 0 and x < self.nbTilesX and y >= 0 and y < self.nbTilesY) then
			...
			if(indexFrameTexture > -1) then 
				...
				if(indexTextureSequence > -1) then 
					...
					if(t.texture.sequence ~= seq.Name) then 
						...
						local currentFrameTexture = self.tabCurrentTextureFrameForTile[indexTileInTabs][seq.Name]
						if(not currentFrameTexture) then currentFrameTexture = 1 end
						print(currentFrameTexture)
						
						t.texture.currentFrame = currentFrameTexture
						t.texture:play()
					end
					t.texture.isVisible = true
				else
		...
		if(self.imageObjectSet) then 
			...
			if(self.tabCurrentObjectFrameForTile) then self.tabCurrentObjectFrameForTile[indexTileInTabs] = t.imageObject.currentFrame end
			...
			if(indexFrameObject >-1) then 
				...
				if(indexObjectSequence > -1) then 
					...
					if(t.imageObject.sequence ~= seq.Name) then 
						...
						local currentFrameObject = self.tabCurrentObjectFrameForTile[indexTileInTabs]
						if(not currentFrameObject) then currentFrameObject = 1 end
						t.imageObject.currentFrame = currentFrameObject
						t.imageObject:play()
					end
					t.imageObject.isVisible = true
				else
		...
		if(self.tabCurrentTextureFrameForTile) then self.tabCurrentTextureFrameForTile[t.currentIndexInTabs] = t.texture.currentFrame end
...
function TilesMap:loadTilesIndexTextureSequences()
	...
	if(self.tabTilesTextureSequences) then 
		self.tabCurrentTextureFrameForTile = {}
		local nbTotalTiles = self.nbTilesX * self.nbTilesY
		for j = 1, nbTotalTiles do 
			self.tabCurrentTextureFrameForTile[j] = {}
			for i = 1, self.infos.TextureSequenceCount do 
				self.tabCurrentTextureFrameForTile[j][self.infos.TextureSequences[i]] = 1
			end
		end
...
function TilesMap:loadTilesIndexImageObjectSequences()
	...
	if(self.tabTilesObjectSequences) then 
		self.tabCurrentObjectFrameForTile = {}
		-- for i = 1, #self.tabTilesObjectSequences do 
			-- table.insert(self.tabCurrentObjectFrameForTile,1)
		-- end
	end
end	
